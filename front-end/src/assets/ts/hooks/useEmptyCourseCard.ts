...
export default function useEmptyCourseCard(grades: string[], lessonNum: number, queryDate: string, isDateMode: boolean) {
    const store = useStore();
    const message = useMessage();
    const dialog = useDialog();

    const queryWhatDay = computed<number>(() => getIsoWeekDay(dayjs(queryDate)));
    const queryWhatDayStr = computed<string>(() => store.getWhatDayStr(queryWhatDay.value));

    function addInfoInThisBlockIntoStore() {
        store.editor.lessonNum = lessonNum;
    }
    ...
    return {contextMenuItems, onContextMenu};
}
